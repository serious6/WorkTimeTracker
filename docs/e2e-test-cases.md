...
| P4 | `P4: an archived project leaves the tracking selections and can be restored` | Two projects exist and one has a time entry | The user archives that project, checks the tracking picker and the entry dialog, then unarchives it | The archived project is marked "Archived" on the Projects page and keeps its total and entries, it is offered in no tracking selection, and unarchiving brings it back |
| P5 | `P5: an overdue budget warns during tracking without blocking it` | A project has three tracked hours and a two hour budget | The user selects the project in the tracking card and starts the timer | A "Budget overdue" status names the exceeded budget for the selected and for the running project, the timer starts and stops normally, and a project without an overdue budget shows no warning |
