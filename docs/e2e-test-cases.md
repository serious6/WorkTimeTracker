...
| 23 | `returns to the login page when the session expires` | The user is signed in and a project exists | The stored session is aged, first past its idle timeout and then past its absolute lifetime, and the user opens "Budgets" | Both expiries return the application to the login page; after signing in again the data is shown unchanged |
