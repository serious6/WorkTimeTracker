...
| E18 | `E18: stores a session that follows a rounded up session` | A session of 35 seconds was stopped and stored as "0h 01m", whose entry ends ahead of the clock | The user starts the timer again at once, tracks 40 seconds and stops it | The follow-up session is measured from the moment it was started, so "0h 01m" is added again and the day total is "0h 02m" |
