...
| 27 | `lets a password manager fill and submit the sign-in form` | The user is signed out and the login page is shown | The window title and the form identity are checked, the fields are filled programmatically like an autofill and the form is submitted with <kbd>Enter</kbd> | The title reads "WorkTimeTracker — Sign in", the form is named `login-form` with the autocomplete tokens `username` and `current-password`, and the filled credentials sign the user in |
...
| E18 | `E18: stores a session that follows a rounded up session` | An entry of 08:00–09:00 exists and a session of 35 seconds was stopped at 09:00, so its stored minute cannot grow into the past and ends ahead of the clock | The user starts the timer again at once, tracks 40 seconds and stops it | The follow-up session is measured from the moment it was started, so "0h 01m" is added again and the day total is "1h 02m" |
