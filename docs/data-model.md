...
the signed-in user in memory only, so a restart returns to the login page. Each session also holds
the label of the webview it was started from, and a command only accepts the session id from that
window.
