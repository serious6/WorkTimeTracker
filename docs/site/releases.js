import { RELEASES_PAGE, formatBytes, inferPlatform, loadReleases, releaseState } from './release-data.js'
...
function render(releases, stale) {
  ...
  const assets = release.assets.map((asset) => `<a class="asset" href="${escape(asset.browser_download_url)}"><span class="asset-icon" aria-hidden="true">↓</span><div><strong>${escape(inferPlatform(asset.name))} · ${escape(asset.name)}</strong><span class="asset-meta">${formatBytes(asset.size)} · ${asset.download_count} download${asset.download_count === 1 ? '' : 's'}</span></div><span class="download-arrow" aria-hidden="true">↓</span></a>`).join('')
...
function renderError(error) {
  content.setAttribute('aria-busy', 'false')
  const rateLimited = error.message === '403'
