...
an audit. Expired sessions are dropped whenever the map is read, so it stays bounded.

A session also records the label of the webview it was started from, and a command only resolves it
when the call arrives from that webview. The id alone is a bearer token, so binding it to its window
makes a leaked id useless everywhere else: a replay from another window answers `notSignedIn`
without extending the idle timeout, and only the owning window can end the session. The frontend
