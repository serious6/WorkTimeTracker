...
import { totalMinutes } from '@/features/dashboard/metrics'
...
export function TimeEntriesPage() {
  ...
  const [filter, setFilter] = useState<string>(projectFilter ? `${projectFilter}` : '')
  const [dialogOpen, setDialogOpen] = useState(false)

  const days = useMemo(() => {
    const filtered = filter ? entries.filter((entry) => entry.projectId === Number(filter)) : entries
    ...
    <div className="space-y-5">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">All tracked time, newest day first.</p>
        </div>
          ...
          <Card key={day}>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Total: {formatDuration(totalMinutes(dayEntries, now))}
              </p>
            </CardHeader>
            <CardContent>
              <TimeEntryList
